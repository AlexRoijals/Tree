...
public class Deformable : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (baseMesh.normals == null || baseMesh.normals.Length == 0)
        {
            ...
            Debug.LogWarning("Missing vertex normals, dynamic normal will be used. Either assign a value to the dynamic normal or runtime normals will apply.");
        }
        ...
        for (i = 0; i < _mesh0.vertexCount; i++)
        {
            ...
            for (j = 0; j < _deformers.Count; j++)
            {
                ...
                if (!_deformers[j].Transformed || !_deformers[j].enabled)
                    continue;
                // calculate the distance between current vertex and the current localized deformer position (subtracting the deformer's spacing)
                vtxDist = Vector3.Distance(verts[i], _localDeformerPosition[j]) - _deformers[j].Spacing; // _heatSources[j].GetDistance(localizedHeatSources[j] - verts[i]);ing;// _heatSources[j].GetDistance(localizedHeatSources[j] - verts[i]);
            ...
            if (_restore)
                ...
                clrs[i].a = Mathf.Min(clrs[i].a, Mathf.Clamp01(clrs[i].a - (viewDistance - remapDist) * Time.deltaTime * fadeSpeed));
        ...
        for (i = 0; i < _mesh0.vertexCount; i++)
        {
            ...
            for (j = 0; j < _deformers.Count; j++)
            {
                ...
                if (!_deformers[j].Transformed || !_deformers[j].enabled)
                    continue;
                // calculate the distance between current vertex and the current localized deformer position (subtracting the deformer's spacing)
                vtxDist = Vector3.Distance(verts[i], _localDeformerPosition[j]) - _deformers[j].Spacing; // _heatSources[j].GetDistance(localizedHeatSources[j] - verts[i]);
                // if distance to vertex is above minimum distance for deformers to apply to the deformable
                if (vtxDist > _deformerMinDistance)
                    continue;
                // if the distance to the deformer is lower than the minDist set it as new minDist and save the index of the deformer
                if (vtxDist < minDist)
                {
                    minDist = vtxDist;
                    closestDeformer = j;
    ...
    /*
    List<int> _triangleSubrange = new List<int>(new int[] { 1500, 1510, 1520, 1530, 1540, 1550, 1560, 1570, 1580, 1590, 1600, 1610, 1620, 1630, 1640, 1650, 1660, 1670, 1680, 1690 });
    private bool DisplaceByMeshNormal(Vector3[] verts, Vector3[] normals, Color[] clrs, List<int> subrange)
    {
        modified = false;
        orient = Mathf.Clamp(_fixedNormalDir, -1.0f, 1.0f);
        minDist = 0.0f;
        ...
        foreach(int t in subrange)
        {
            for (int k = 0; k < 3; k++)
            {
                ...
                for (j = 0; j < _deformers.Count; j++)
                {
                    vtxDist = Vector3.Distance(verts[i], _localizedHeatPosition[j]) - _deformers[j].Spacing;// _heatSources[j].GetDistance(localizedHeatSources[j] - verts[i]);
                    // if distance to vertex is above MaxDistance the source is s
                    if (vtxDist > _deformerMinDistance)
                        continue;
                    if (vtxDist < minDist)
                        minDist = vtxDist;
                ...
                modified = true;
                // remapDist = this.Remap(minDist, 0.0f, 3.0f, 0.0f, 1.0f);
                remapDist = this.Remap(minDist, 0.0f, _effectRange, 0.0f, 1.0f);
                ...
                // displace the vertex by the mesh's normal
                verts[i] = _restoreVertices[i] + (normals[i] * (1.0f - clrs[i].a) * orient * _deformMagnifier);
            }
        }
